...
unsigned int nTargetSpacing = 1 * 60; // Flux - 1 minute blocks
static const int64_t nInterval = nTargetTimespan / nTargetSpacing;
static const int64_t nDiffChangeTarget = 1;

unsigned int nStakeMinAge = 8 * 60 * 60; // 8 hours
unsigned int nStakeMaxAge = 60 * 60 * 24 * 30;  // 30 days
unsigned int nModifierInterval = 10 * 60; // Flux - time to elapse before new modifier is computed
...
bool CheckDiskSpace(uint64_t nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "Flux", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        block.nNonce   = 93180730;
        
        assert(block.hashMerkleRoot == uint256("0x993fe7e779dc30991f0dfa43b58aa4e77deff93dc81926f3f56a79875b1f12a5"));
